...
└─────────────────────────────────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────┐
│ TiledUIView<Item, Cell> (UIView)                            │
│   - items: Deque<Item>                                      │
│   - cellBuilder: (Item, CellState) -> Cell                  │
│   - sizingHostingController: UIHostingController<Cell?>     │
│                                                             │
│   Applies changes generated by comparing displayed items    │
│   with the latest [Item] snapshot:                          │
│   - .replace → reload all                                   │
...
struct ChatView: View {
  ...
  var body: some View {
    ...
    .task {
      messages = await fetchInitialMessages()
    }
  }
...
### Solution: Snapshot Diffing

TiledView solves this by comparing the currently displayed items with the latest
`[Item]` snapshot inside `TiledUIView`:
...
│   │   ├── TiledUIView           - Main UIView component
